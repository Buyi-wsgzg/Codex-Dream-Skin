...
- Fiona 首页标题中的项目名标签改用深紫文字、高不透明浅粉底色并移除继承的文字阴影，避免项目名与横幅高光区融在一起。
- Arina 与 Fiona 经典主题会根据主窗口可用尺寸计算统一响应式倍率；从普通窗口切换到 Full Window 时，横幅、建议卡片、输入框、拍立得、蝴蝶结与其间距会同步放大并重新定位，文字倍率单独封顶以避免长标题失控换行。
